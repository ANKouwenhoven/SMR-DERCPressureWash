...
ARDUINO_ENABLED     = True           # Set to True when Arduino is connected
...
class App(tk.Tk):
    ...
    def _start_scan(self):
        ...
        if ARDUINO_ENABLED:
            ...
            # Hide manual button — Arduino handles rotation automatically
            self.btn_continue.config(state="disabled")
            self.rotate_info.config(text="Arduino controlling\ntable rotation\nautomatically.")
            self._set_status("Scan started.\nArduino rotating table\nautomatically.")
        else:
            # Show manual button
    ...
    def _do_scan_step(self):
        ...
        try:
            ...
            if ARDUINO_ENABLED:
                self._set_status(f"Photo taken at {self.current_angle:.1f}°\nRotating table...")
                self.rotate_info.config(text=f"Rotating to:\n{self.current_angle + DEGREES_PER_STEP:.1f}°")
                # Rotate in background thread then auto-trigger next step
                def rotate_and_continue():
                    self.arduino.rotate(DEGREES_PER_STEP)
                    self.current_angle += DEGREES_PER_STEP
                    self.after(0, self._do_scan_step)
                threading.Thread(target=rotate_and_continue, daemon=True).start()
            else:
                self._set_status(
    ...
    def _on_rotated(self):
        """User confirms table has been rotated (manual mode only)."""
