...
CONE_HALF_ANGLE_DEG = 3.0      # spray half-angle — adjust to real nozzle
CONE_SEGMENTS       = 20        # triangle fan resolution (sides of cone)
PATCH_DISC_SEGS     = 12        # triangle fan resolution for surface discs
PATCH_SAMPLE_STEP_M = 0.005     # sample stripe every 5 mm for raycasting
...
class CleaningVisualizer:
    ...
    def commit_stripe(self,
        ...
        for t in ts:
            ...
            # Disc radius: spread at this distance — minimum 15mm so patches are always visible
            disc_r = max(0.015, hit_dist * np.tan(np.deg2rad(self._half_angle)))
