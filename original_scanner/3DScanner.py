...
Modified to add live camera preview on Linux
"""
...
class Scan():
    ...
    def getPreviewFrame(self):
        """Get a single color frame for live preview."""
...
class App(tk.Tk):
    ...
    def startPreview(self):
        """Start the camera pipeline and begin live preview loop."""
        try:
            self.preview_scan = Scan(
                int(self.dictionary["widthFrame"]),
                int(self.dictionary["heightFrame"]),
                30, 1, 0
            )
            self.preview_scan.startPipeline()
            self.preview_running = True
            self.updatePreview()
            print("Live preview started")
        except Exception as e:
            print(f"Could not start preview: {e}")

    def updatePreview(self):
        """Continuously update the canvas with the latest camera frame."""
        if self.preview_running and not self.is_scanning:
            try:
                frame = self.preview_scan.getPreviewFrame()
                if frame is not None:
                    self.frames["StartPage"].showImage(frame)
            except Exception as e:
                print(f"Preview error: {e}")
            self.after(33, self.updatePreview)  # ~30fps

    def stopPreview(self):
        """Stop the live preview and release the camera."""
        self.preview_running = False
        try:
            self.preview_scan.stopPipeline()
        except:
            pass
    ...
    def startScan(self):
        ...
        self.step_size = int(self.dictionary["stepSize"])
        # Calculate angle per step (stepSize is in stepper motor steps, convert to degrees)
        # With gear ratio 6 and 2048 steps/rev: degrees = (steps * 360) / (2048 * 6)
        self.angle_per_step = (self.step_size * 360) / (2048 * 6)
        self.doNextScanStep()

    def doNextScanStep(self):
        """Take a photo at current angle, then wait for user to rotate table."""
        try:
            ...
            self.frames["StartPage"].Progress(self.current_angle)
            self.scan.processFoto(self.current_angle)
            self.update()

            # Ask user to rotate table
            self.frames["StartPage"].showRotatePrompt(
                self.current_angle,
                self.angle_per_step,
                self.onTableRotated,
                self.stopScanEarly
    ...
    def onTableRotated(self):
        """Called when user confirms they have rotated the table."""
        self.current_angle += self.angle_per_step
        self.frames["StartPage"].hideRotatePrompt()
        self.doNextScanStep()

    def stopScanEarly(self):
        """Called when user wants to stop the scan early."""
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.load = Image.fromarray(np.zeros(shape=(480, 848, 3), dtype=np.uint8), 'RGB')
    ...
    def showRotatePrompt(self, current_angle, step_degrees, on_confirm, on_stop):
        """Show a prompt asking user to rotate the table."""
        self.rotateFrame = tk.Frame(self)
        msg = f"Angle: {current_angle:.1f}° — Rotate table by {step_degrees:.1f}° then click Continue"
        self.rotateLabel = tk.Label(self.rotateFrame, text=msg, font=("Arial", 10))
        self.rotateLabel.pack(side="left", padx=10)
        self.continueButton = tk.Button(self.rotateFrame, text="Continue", bg="#4CAF50", fg="white",
                                        width=12, command=on_confirm)
        self.continueButton.pack(side="left", padx=5)
        self.stopEarlyButton = tk.Button(self.rotateFrame, text="Stop Scan", bg="#f44336", fg="white",
                                         width=12, command=on_stop)
        self.stopEarlyButton.pack(side="left", padx=5)
        self.rotateFrame.grid(row=2, column=1, pady=5)

    def hideRotatePrompt(self):
        """Remove the rotate prompt."""
        if hasattr(self, 'rotateFrame'):
            self.rotateFrame.grid_forget()
            self.rotateFrame.destroy()
...
class SettingsPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.BasicSettingsFrame.pack(side="left", fill="both")
